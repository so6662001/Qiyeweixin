...
> **v9 微调**：**OCR / 视觉理解统一到通义千问（Qwen-VL 系列）**——移除阿里云通用 OCR / 腾讯云票据 OCR / PaddleOCR 等额外依赖；询价图片解析、扫描 PDF、付款凭证 OCR 全部走 Qwen-VL。
...
| 维度 | v3 | v4 | v5 | v6 | v7 | v8 | **v9** |
|---|---|---|---|---|---|---|---|
| 询价输入 | 文字 | 文字/图/Excel/PDF | 同 | 同 | 同 | 同 | 同 |
| 询价字段 | 5 | 7 | 9 | 同 | 同 | 同 | 同 |
| NLU 策略 | LLM | LLM | KB+默认+溯源 | 同 | 同 | 同 | 同 |
| 报价 | 销售单干 | 同 | 同 | 三档协作 | 同 | 同 | 同 |
| 客户画像 | — | — | 偏好 | 完整画像 | 同 | 同 | 同 |
| 库存匹配 | — | — | — | 多源组合 | 同 | 同 | 同 |
| 报价策略 | — | — | — | 11 策略 | 同 | 同 | 同 |
| 行为干预 | — | — | — | 5 层 | 同 | 同 | 同 |
| **议价** | — | — | — | — | 多轮状态机 + 让步曲线 | 同 | 同 |
| **让步货币** | — | — | — | — | 8 维（价/量/时/运/期/质/赠/组） | 同 | 同 |
| **整单优化** | — | — | — | — | 跨项让步分配 + 整单底线 | 同 + 整单改量校验 | 同 |
| **议价 LLM** | — | — | — | — | 带预算的议价话术 + 情绪检测 | 同 | 同 |
| **议价审计** | — | — | — | — | 全轮 offer + 销售决策落库 | 同 | 同 |
| **报价生命周期**（v8 新） | — | — | — | — | — | **DRAFT/ACTIVE/AMENDING/LOCKED/EXPIRED/SUPERSEDED + 版本化** | 同 |
| **改量决策**（v8 新） | — | — | — | — | — | **AUTO / ASSISTED / CUSTOMER_CHOICE / MANUAL / REJECT 五档** | 同 |
| **跌穿 MOQ**（v8 新） | — | — | — | — | — | **客户选项卡片** | 同 |
| **锁后改量**（v8 新） | — | — | — | — | — | **必走审批工单** | 同 |
| **改量频次防护**（v8 新） | — | — | — | — | — | **每 Quote / Session / 日多级阈值** | 同 |
| **视觉 OCR 供应商**（v9 新） | — | — | — | — | — | — | **统一通义千问 Qwen-VL（移除 PaddleOCR / 阿里 / 腾讯 OCR）** |
| **DashScope 集成**（v9 新） | — | — | — | — | — | — | **企业 SLA + 配额 + 子账号 + 计费监控** |
