...
| **44（v23 新）** | 语音输入 | **Voice Input / ASR（5.72）**：通义听悟/Qwen-Audio 转写 → 文字询价流程 |
...
ASR 转写（通义听悟 / Qwen-Audio / 微信语音转文字）
   ↓ 文字
走正常文字询价/意图流程
```
...
1. 销售粘性窗口默认 30 天？是否企业可配？
2. 壁厚/单重替代是否工程类用途一律禁用？
3. 促销优惠与议价让步叠加规则（能否同时用）？
4. 语音 ASR 用通义听悟 vs Qwen-Audio vs 微信原生转写？
5. 批次匹配默认 FIFO 还是销售指定？
6. 自动跟进频率上限（每周/每月每客户）？
...
| **语音输入**（v23 新） | … | — | — | **ASR 转写（通义听悟/Qwen-Audio）** |
