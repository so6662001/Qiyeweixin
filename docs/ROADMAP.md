...
| 里程碑 | 内容 |
|---|---|
| M50 | 小程序一期（报价编辑 + 留货 + 微信支付定金） |
| M51 | 小程序二期（查单据 + 订阅消息） |
| M52 | 小程序三期（询价入口 + 我的 + 白标可选） |
| M47 | 计费与配额（SaaS 套餐 + Qwen-VL/讯飞配额 + 文件保留计费） |

- **目标**：小程序重交互 + SaaS 商业化。
- **难度**：高。身份打通（unionid）、微信支付分账、多端一致。
- **风险**：交易类目审核、支付资质、unionid 打通（外部前置）。
- **退出标准**：UAT-16（投递双路径）+ 小程序端报价/留货/支付/查单据通过；配额计费可用。
...
- **Phase 6 小程序**：报价/交易后端（Phase 2/3）稳定后，小程序作为前端并行开发（BFF 复用后端）
...
| Phase 6 | 小程序流畅编辑报价/留货/支付/查单据；SaaS 计费 |
