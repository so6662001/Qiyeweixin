...
- **粘性窗口默认 30 天，企业可配（v24 确认）**；是否启用粘性可配；销售评分差可解除粘性
...
- 与用途场景（v11）：工程类用途**默认禁**壁厚/单重替代，**企业可配放开（v24）**
- 成分/标准兼容（5.70）硬约束仍适用
...
- 促销后仍须过全成本红线
- **促销 + 议价可叠加（v24）**：客户先享明牌促销，再议价让步；叠加后仍须过全成本红线（v16）
  - 顺序：先应用促销（明牌）→ 客户砍价进议价（在促销价基础上让步）
  - 叠加上限：全成本红线兜底，破线转销售
...
ASR 转写【科大讯飞 ASR — v24 确定】→ 文字 →
Steel KB 实体校正（5.36）→ 低置信回显确认 → 正常意图流程
```

- **ASR 供应商：科大讯飞**（v24）；视觉 OCR 仍用 Qwen-VL（v9）→ 语音与视觉分供应商
- 科大讯飞优势：中文/方言识别强，钢铁行业可定制热词（"螺四/螺纹/抄牌"等）
- 钢铁黑话/方言：讯飞热词 + 转写后经 KB 校正
- 低置信度 → "我听到的是：螺纹 HRB400 Φ25 50吨送武汉，对吗？"
- 语音计入 ASR 配额（v15 计费）；科大讯飞按调用计费，纳入套餐配额
- 新表 `voice_transcription_log`
- 出口白名单（v9 部署）增加 科大讯飞 ASR endpoint

**5.72.X 语音 + 多规格（v25）**
- 一条语音可含多个规格 → 转写后交多规格切分（5.12 + 4.2）
- 共享要素提取：目的地/交货日期/客户身份对所有规格生效；规格本体各自独立
- **错误率叠加 → 强制逐规格回显确认**（语音误差 × 多规格，不可静默提交）
- 部分识别成功：已识别的正常确认，模糊的单独反问
- 超长语音/十几个规格 → 提示改发 Excel/文件 或分条
- 文字多规格走相同切分逻辑（无 ASR 步骤）
...
- 留货/下单匹配批次（v24 规则）：
  - **价格相同 → FIFO**（先进先出，清老货）
  - 价格不同 → 按报价策略/库龄优惠（老货优先出清，5.71 库龄优惠）
  - 销售可手动指定批次（覆盖默认）
...
**5.74.2 频率控制（防骚扰，v24：按采购频率自适应）**

- **跟进频率按客户采购频率自适应**（v24）：
  - 学习客户历史采购周期（自我学习 5.40 / 周期复购）
  - 月采购客户 → 月度跟进节奏；周采购客户 → 更频繁
  - 不设统一固定上限，匹配各客户节奏
- 仍有安全上限（防异常，可配）+ 与软干预（v6）协同：ghost 客户不过度跟进
- 静默时段（夜间不发）
- 客户可退订
